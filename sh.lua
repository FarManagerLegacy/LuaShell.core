...
local function assert2 (expr, ...) -- "smart" assert, to make error point to proper level
  if expr then
    return expr, ...
  end
  local level = 1
  repeat
    level = level+1
  until debug.getinfo(level, "S").source~=src --todo ?getfenv
  error(..., level)
end
...
local findScript, loadFile --fwd. decl.
...
local function indexScript (cache, name, level)
  ...
  if O.debug then
    ...
    ret, ret2 = callWithErrMsg(msg_runerr.."\1\n", fn, name)
  end
  if not O.debug then
  ...
  function mt_autoload:__index (name)
    ...
    elseif not blacklisted[name] then
      ...
      if rawget(_sh, "autoload") then
        ...
        return indexScript(self,name,2)
      end
    end
  end
end
