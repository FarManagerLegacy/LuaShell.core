local function error (msg, level)
  sh.sharedUtils.error(msg, level+1)
end

local function assert2 (expr, ...) -- to make error point to proper level
  if expr then
    return expr, ...
  end
  error(..., 3)
end
local function callWithErrMsg (msg, fn, ...)
  return select(2, assert2(xpcall(fn, function (err2) -- rewrite err msg
    return msg..err2
  end, ...)))
end

local function eval (expr, env, ...)
  if not expr then error("eval: expression expected", 2) end
  if env and type(env)~="table" then
    error("eval: bad argument #2 (table expected, got "..type(env)..")", 2)
  end
  local fn,err1 = sh.loadstring(expr, "eval%s...", env)
  local msg_runerr = "eval:\n"..expr.."\n\1\n"
  if not fn then
    error(msg_runerr..err1, 2)
  end
  if sh._shared.options.debug then
    return fn(...)
  end
  return callWithErrMsg(msg_runerr, fn, ...)
end
...
if _cmdline=="" then
  ...
  local function saveRes (...)
    ...
    res.short = sh.sharedUtils.shortret(res)
