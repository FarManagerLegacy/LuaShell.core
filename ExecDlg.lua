...
local function importEnv (env, name)
  ...
  if not success then
    return err--"error processing env init script: "..err.."\n\1\n"..path--??
  end
  env._filename = nil
...
local function setStatus (hDlg,data)
  ...
  if not err then
    ...
    err = not success and ret
  end
  data.ret = success and ret
  data.err = err
  ...
  local text = success and (ret.n>0 and U.shortret(ret) or "") or err-- and err:gsub("\n", " ")
    ...
    if idx>0 then
      ...
      local err = sh.execCmdline(cmdline, options, function (runtimeErr) -- onDone
        ...
        if options.reopen or runtimeErr then --rerun!!
          ...
          Data[hDlg:rawhandle()] = nil; return far.DialogFree(hDlg) -- cleanup
        end
      ...
      Data[hDlg:rawhandle()] = nil; far.DialogFree(hDlg) -- cleanup
    end
    return true
