...
В комплект поставки LuaShell входит набор скриптов, не являющихся его неотъемлемой его частью,
но спонтанно развившихся со временем в его экосистеме.  
Они размещены в директории `Macros\utils\std`.

Кроме того, отдельно распространяется набор со скриптами/макросами других авторов,
приспособленными для использования в LuaShell, такие скрипты размещены в
`Macros\utils\3rd-party`.

В данной справке упоминаются лишь отдельные из дополнительных скриптов-утилит,
а для получения более подробной информации следует смотреть скрипты непосредственно —
...
Для установки содержимое архива (директории `scripts` и `utils`) достаточно распаковать в
`%FARPROFILE%\Macros\` и перезагрузить макросы.
