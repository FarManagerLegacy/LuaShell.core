...
local function smartTrace (err) -- "smart" trace, to make traceback exclude irrelevant levels
  local level = 2
  if sh._shared.errLevel then -- the error explicitly thrown by our code (sh.sharedUtils.error)
    level = sh._shared.errLevel + 2
  end
  if type(err)~="string" then
    err = err==nil and "<no message>" or tostring(err)
  end
  local msg = cutTail(
...
local function execCmdline (cmdline, opts, done, init)
  ...
  elseif not opts.expr_only and (opts.noexpr or cmdline:sub(1,1)=='"') then
    ...
    env = env or {}
    f,err = sh.loadstring(cmdline, "eval%s...", env)
  end
  ...
  local trace = _shared.options.debug and debug.traceback or smartTrace --????? or postmacro(ErrMessage
  local function run ()
    if opts.echo_force then print(); U.SetUserScreen() end -- print prompt only
    SetDir(fardir,_dir)
    if isExpr then arg = {} end --??
    local success,ret = ppack(xpcall(f, trace, unpack(arg)))
    ...
    if success then
      ...
      saveRes(nil); err = ret
      err = err==nil and "<no message>" or err
      if type(err)=="string" then
        err = err:gsub("\n\t", "\n   ")
      end
      ErrMessage(err)
